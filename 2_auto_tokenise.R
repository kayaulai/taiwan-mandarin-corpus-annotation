...
file_name <- "NCCU-TM013-CN-FFF.csv"  # Only change the file path to the input file in <1_auto_split>
...
conjunct<-function(p){
  for (i in seq(1,length(p))){
    ...
    p[[i]]=str_replace_all(p[[i]],"< L", "<L")
    p[[i]]=str_replace_all(p[[i]],"(\\d) >","\\1>")
    p[[i]]=str_replace_all(p[[i]],"< X", "<X")
    p[[i]]=str_replace_all(p[[i]],"X >","X>")


    p[[i]]=str_replace_all(p[[i]],"<X([^ ])", "<X \\1")
    p[[i]]=str_replace_all(p[[i]],"([^ ])X>", "\\1 X>")
    p[[i]]=str_replace_all(p[[i]],"<L(\\d)([^ ])", "<L\\1 \\2")
    p[[i]]=str_replace_all(p[[i]],"([^ ])L(\\d)>", "\\1 L\\2>")


    p[[i]]=str_replace_all(p[[i]],"X X","XX")
    ...
    #Bonus
    p[[i]]=str_replace_all(p[[i]],"覺 得","覺得")
  }
  return(p)
}
